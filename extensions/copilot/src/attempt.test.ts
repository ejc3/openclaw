...
function expectTranscriptCredentialSafety(instructions: string, strict = false): void {
  ...
  expect(instructions.includes("host-owned masked credential entry")).toBe(strict);
  expect(instructions).toContain("Never ask users to paste passwords or reusable credentials");
  expect(instructions).toContain(
...
describe("runCopilotAttempt", () => {
  ...
  describe("workspace bootstrap (systemMessage)", () => {
    ...
    it.each([undefined, true, false])(
      "applies transcript policy %s to new SDK sessions",
      async (allow) => {
        const sdk = makeFakeSdk();
        const pool = makeFakePool(sdk);

        await runCopilotAttempt(
          makeParams({ config: { security: { allowCredentialsInTranscript: allow } } }),
          { pool },
        );
        ...
        expectTranscriptCredentialSafety(cfg.systemMessage?.content ?? "", allow === false);
      },
    );
    ...
    it.each([undefined, true, false])(
      "forwards bootstrap and transcript policy %s to resumed SDK sessions",
      async (allow) => {
        const rendered = "# Project Context\n## /ws/SOUL.md\n\nSoul voice goes here.";
        workspaceBootstrapMock.resolveCopilotWorkspaceBootstrapContext.mockResolvedValueOnce({
          bootstrapFiles: [],
        ...
        await runCopilotAttempt(
          makeParams({
            config: { security: { allowCredentialsInTranscript: allow } },
            initialReplayState: { sdkSessionId: "sess-resume-1" },
          } as never),
          { pool },
        );
