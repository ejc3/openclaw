...
export function buildCredentialSafetyPrompt(
  secretsToolName?: string,
  options?: { allowCredentialsInTranscript?: boolean },
): string {
  return [
    ...
    "Never ask users to paste passwords or reusable credentials into chat.",
    // Transcript handling is permissive by default; protected-store rules below
    // still apply whenever that tool is callable. Guided setup opts in explicitly.
    ...(options?.allowCredentialsInTranscript === false
      ? [
