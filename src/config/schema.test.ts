...
describe("config schema", () => {
  ...
  it.each([true, false])(
    "accepts credential transcript policy %s in the runtime zod schema",
    (allow) => {
      ...
      const parsed = OpenClawSchema.parse({
        security: { allowCredentialsInTranscript: allow },
      });

      expect(parsed.security?.allowCredentialsInTranscript).toBe(allow);
      expect(
