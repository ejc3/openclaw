...
`security.allowCredentialsInTranscript` controls the shared prompt policy for
handling credentials that a user supplies in conversation. It defaults to `true`.
Mandatory masked entry is opt-in, not the default.

| Value           | Ordinary agent, Codex, and Copilot prompts                                                                                                  |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| Unset or `true` | Omit the blanket masked-entry and visible-text restriction                                                                                  |
| `false`         | Require protected entry for reusable secrets and instruct the agent to keep them out of replies, tool arguments, URLs, logs, and shell text |

To enable the stricter policy:

```json5
{
  security: {
    allowCredentialsInTranscript: false,
  },
}
```

Both modes retain instructions not to solicit passwords or reusable credentials
in chat. Supported private short-lived login-code handoffs and OAuth callbacks
remain available. The [secrets tool](/tools/secrets) remains protected in both
modes, including its masked input, SecretRefs, and allowed-host egress controls.
Guided system-agent setup always uses the stricter policy because it owns those
protected entry paths.

This is prompt guidance, not automatic transcript masking or redaction. Allowing
credential handling does not guarantee a model will comply, provide browser
autofill, or override workspace instructions, tool permissions, or service rules.
Values supplied through ordinary chat or tool arguments can reach model context,
transcripts, logs, and configured transcript-derived memory. Prefer existing
signed-in sessions or protected entry when available.

The setting applies across ordinary sessions and all authorized senders, not just
owners or individual turns. It does not change sender authorization. Prompt
builders use the current setting on the next attempt; existing live incognito
Codex threads require a new conversation when their policy changes.
