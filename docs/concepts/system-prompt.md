...
- **Safety**: short guardrail reminder against power-seeking behavior or bypassing oversight, plus credential handling: do not solicit reusable secrets in chat; allow private short-lived sign-in handoffs. Mandatory masked entry is opt-in via `security.allowCredentialsInTranscript: false`; protected secrets-tool guidance remains in both modes.
