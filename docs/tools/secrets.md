...
policy removes `secrets`. These tool-specific protections apply regardless of
[`security.allowCredentialsInTranscript`](/gateway/configuration-reference#credential-transcript-safety).
That setting controls the ordinary-agent prompt mandate, not this tool's masking,
storage, or egress restrictions. The agent must not ask for reusable secrets in chat.
